...
def evaluate_model(
    ...
    return metrics, report, matrix, stats, model_params
...
def extract_model_params(model):
    ...
    if hasattr(model, "named_steps"):
        ...
        if tfidf is not None:
            params.update({
                "tfidf__max_features": tfidf.max_features,
                "tfidf__ngram_range": tfidf.ngram_range,
                "tfidf__min_df": tfidf.min_df,
                "tfidf__max_df": tfidf.max_df,
                "tfidf__sublinear_tf": tfidf.sublinear_tf,
            })
        if clf is not None:
            params.update({
                ...
                "clf__max_samples": clf.max_samples,
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Random Forest sentiment model.")
    parser.add_argument(
        "--data-dir",
        "--data",
        dest="data_dir",
    ...
    metrics, report, matrix, stats, model_params = evaluate_model(
    ...
    matrix_path.write_text(
        matrix_to_svg(matrix, "Random Forest Sentiment Confusion Matrix"),
        encoding="utf-8",
    )
...
def matrix_to_svg(matrix, title):
    ...
    for row in range(len(LABELS)):
        for col in range(len(LABELS)):
            ...
            red = 255
            green = int(245 - 185 * intensity)
            blue = int(245 - 105 * intensity)
